...
export const Projects = () => {
  ...
  if (isLoading) return <div className="text-center text-white py-20">Loading projects...</div>;
  if (error) return <div className="text-center text-red-500 py-20">Error loading projects</div>;

  // Fallback if no projects yet (optional, or just show empty)
  if (!projects || projects.length === 0) {
      return (
        <section id="projects" className="min-h-screen flex items-center bg-slate-900 py-20">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-center">
                <h2 className="text-4xl md:text-5xl font-bold mb-4 text-white">No projects found.</h2>
            </div>
        </section>
      )
  }
